...
**Drills (Decisions §72, 2026-06-11):** the learn-by-fixing system in [src/challenges/](src/challenges/). A challenge JSON (`public/challenges/<id>.json`) stages a BROKEN graph + §70-style starter, auto-runs it (loader raises `autoRunRequested`; Toolbar's useSimulation effect fires), and the `ChallengeBanner` HUD walks observe → diagnose (MCQ) → fix. Every completed run is scored by the pure evaluator (`evaluate.ts`: metric max/avg per component selector + noCrash) — no AI, no network. Content is engine-verified: `challengeContent.test.ts` runs every shipped challenge headlessly (harness.ts mirrors replaceGraph ids + the setApiContracts BFS) and requires broken-FAILS + knownFix-PASSES at two seeds. Landing entry: `LearnSection.tsx`. To author a new drill: copy `queue-backlog.json`, derive numbers from engine formulas, iterate until `npx vitest run src/challenges/__tests__/challengeContent.test.ts -t "challenge: <id>"` is green, add the index.json entry.
...
| Add a new topic key | Declare in the `TOPICS` record in [src/wiki/topics.ts](src/wiki/topics.ts) with category `component\|config\|concept\|howto\|severity\|userGuide\|reference`; author the body in the matching content module (tooltip = `shortDescription` ≤140 chars; page = markdown `body`). Dynamic config keys auto-resolve via `topicForConfigKey(key)` → `config.${key}`. |
...
| Add a new drill (challenge) | New `public/challenges/<id>.json` matching the `Challenge` contract (§72) — copy `queue-backlog.json`. Numbers must derive from engine formulas; iterate until the content gate is green (`npx vitest run src/challenges/__tests__/challengeContent.test.ts -t "challenge: <id>"`), then add the `index.json` entry. |
| Add a flow diagram to a KB section | Insert `<FlowDiagram chain="type:Label -> type:Label" />` in [system-design-knowledgebase.md](system-design-knowledgebase.md) — types are canvas ComponentTypes + `client`/`generic`; rows via `\|`, failure arrows `-x->`, optional `-?->`. DSL in [FlowDiagram.tsx](src/wiki/components/FlowDiagram.tsx). |
...
8. **Phase 0.8** — §40–§44 SIMFID Runtime: code-grounded docs for circuit breaker state machine, retry storm amplification, backpressure propagation, wire-level config, traffic profile semantics. Every subsection cites src/engine/*.ts file:line. Explicit fan-in caveat: `state.metrics` is overwritten per `processComponent` call, so breaker/retry/backpressure signals are last-invocation-biased in fan-in topologies (ForwardResult refactor deferred). Done 2026-04-18. *(The fan-in caveat was later fixed by the §52 refactor; §40.6/§41.2/§42.5 rewritten.)*
9. **Phase 0.9 (2026-06-10, Decisions §69)** — §45–§48 SIMFID Runtime extensions: Kingman G/G/1 queueing (§45), DB read/write split + attribution (§46), capacity estimation / BOTE (§47), calibration anchors (§48). Plus credibility footers (`*Sources: …*`, names only, no URLs) on every §1–§39 section. Done.

KB Phase 0.x complete through §48. **A-content back-fill done 2026-06-10:** all hand-declared topics now have real bodies, authored in per-category modules under [src/wiki/content/topics/](src/wiki/content/topics/) (componentsAndConcepts / configsInfra / configsDataTraffic / newFeatures) and merged into `TOPICS` ahead of the generated tracks. Registry: 170 topics, 0 empty (43 reference + 19 userGuide + 6 howto generated; ~102 hand-written). Config bodies explicitly distinguish engine-live fields from declared-but-ignored ones — see Decisions §69.
