...
- **Rejected:** Clamping `dbArrivalFactor` at 1 (would silently discard fan-out amplification). _Superseded by §63 — round 4 found that without the clamp, default-bucket traffic over-attributes routes; §66 then restored fan-out amplification via a static-multiplier walk so both concerns are addressed simultaneously._ Tracking per-route provenance through the graph (requires disassembling the §52 fan-in refactor — too invasive). Using `totalInboundRps` as the scan multiplier denominator directly (would include default-bucket traffic that by construction has no `TableAccess` info — dilutes the signal). The chosen denominator (`routedDbShareSum` post-scaling) still reflects only the DB's attributable load, consistent across the multiplier and the callout threshold (§60).
...
#### Known limitation — per-route DB arrival scaling under heterogeneous DYNAMIC filtering

`computeDbArrivalFactor` collapses every route reaching a DB to a single scalar. Round 8 (§66) restored fanout amplification via a static `routeStaticAmplificationToNode` walk that reads each chain's `fanout.config.multiplier` — so static amplification differences across routes are now handled correctly. The remaining residual is **heterogeneous DYNAMIC filtering**: one route through a cache (90% hit rate, dynamic), one direct. Both routes share the same observed `totalInboundRps`-derived factor, so the cached route's per-route DB share is over-attributed at high cache hit rates. This affects:
...
Aggregate `errorRate` is bounded by `totalInboundRps` (which reflects the DB's actual load including all dynamic filters), so breakers and backpressure (§52, §54) continue to react correctly to total load — the residual concerns are per-side / per-table attribution, not aggregate control signals.

The remaining fix would require per-chain dynamic-filter modeling with live cache-hit-rate readings; introduces tick-time dependencies that the round-8 static walk deliberately avoids. Deferred until user feedback shows the limitation matters in practice. Original framing was codex round 7 [P2]; round 8 (§66) addressed the static-amplification half, leaving only the dynamic-filter half here.
...
- **Round 7 [P2-CO] follow-up (2026-04-24):** the round-6 fix only seeded `componentEarliestInboundMs` from `pendingInbound` at tick-start, so the FIRST hop receiving deferred traffic emitted with the correct `dispatchedAtTickMs` — but once that target's processor ran and emitted to its OWN downstream IN THE SAME TICK, the next outcome stamped fresh `this.time * 1000` because the in-tick real-delivery branch never wrote back to the map. Cycle `A→B→A→C` exposed it: tick T+1 delivered B→C with stamp T+1, not the original T. The follow-up adds the matching `Math.min` write at the bottom of `emitOutbound`'s non-deferred branch so the timestamp propagates through any number of in-tick hops. New regression test [`fanIn.test.ts`](src/engine/__tests__/fanIn.test.ts) `dispatchedAtTickMs propagates through multi-hop in-tick paths after deferral (round 7 [P2])` pins the chain. Full suite 420/420.

### 66. SIMFID Phase 4 codex round 8 (desktop review) — restore fanout amplification in DB attribution via static-multiplier walk

- **When:** 2026-04-27 (Codex desktop, after the codex CLI rounds 8a-8d timed out on pre-existing tsc errors and were re-routed through the [docs/codex-reviews/](docs/codex-reviews/2026-04-27-phase4-final-convergence/REQUEST.md) handoff system).
- **Context:** Codex desktop confirmed rounds 1-7 fixes hold up but flagged a load-bearing BLOCKER the prior CLI rounds had categorized as a documented limitation. The §63 `Math.min(1, totalInboundRps / entryShareToDb)` cap correctly contained the round-4 default-bucket bleed, but it ALSO silently flattened fanout-amplified routes' DB share down to their seeded entry share. Worked example codex named: route `svc → fanout(multiplier=10) → db` with entry share 100 rps → DB receives 1000 writes. Pre-fix attribution clamped to `100 routed + 270 fallback writes (30% of 900) = 370` against `writeThroughputRps=500` → `writeErrorRate=0`. Aggregate `errorRate = max(read, write, pool) = 0`. Breakers, retry, backpressure (which read `state.metrics.errorRate` per §52) silently miss real saturation in a realistic Discord-style topology. The §61 "Known limitation" claim that aggregate behavior is unaffected is FALSE in this case — the per-side rates ARE the aggregate via `max(...)`.
- **Decision:** New private helper `routeStaticAmplificationToNode(route, targetId)` walks the route's chain prefix from `componentChain[0]` up to (but not including) the target. For each intermediate component, multiplies by `fanout.config.multiplier` (other types stay 1:1 in their static config — cache hit-rate filtering is dynamic and stays handled by `dbArrivalFactor < 1` via the observed `totalInboundRps`). `computeDbArrivalFactor` now divides `totalInboundRps` by `Σ(entryShare × amp)` instead of `Σ entryShare`, and the per-route attribution loops in `computeDbReadWriteBreakdown` and the unindexed-scan path multiply each route's DB share by the same amp factor. The clamp at 1 stays — it now correctly disambiguates "default-bucket bleed" (factor would be > 1 against the routed-amp baseline → clamp keeps routes attributed only at their amplified share, default falls into 70/30) from "fanout amplification" (factor ≈ 1 because total inbound matches the amplified route share → routes get full DB share).
- **Why:** Restores fanout amplification modeling that round 4 traded away while keeping the round-4 default-bucket containment. Aggregate `errorRate` again reflects the DB's true load on Discord-shape and Reddit-fanout-write topologies — breakers / retry / backpressure trip when saturation is real, not just when the unfiltered seeded share looks high. Bounded work — `fanout.multiplier` is a static config field, the walk is O(chain length) per route per DB call, runs only inside the DB processor's attribution loops which were already O(routes) anyway.
- **Rejected:** Walking the chain to model dynamic filter effects (cache hit-rate, server queue drops). The dynamic side stays handled implicitly via `totalInboundRps`'s observed value — adding a second redundant model would create reconciliation bugs. Pure documentation downgrade ("the aggregate IS affected, sorry"). Codex's worked example is concrete enough that ignoring it would be a real false-negative-saturation regression on the headline scenarios.
- **§63 amendment:** the round-4 "Loses fan-out amplification modeling for routes whose chain crosses a fanout node — acceptable; Phase 4.7 fan-out viz is a UI signal" trade-off is hereby overridden. Round 8 restores fanout amplification in attribution; the cap survives unchanged.
- **§61 parenthetical:** round-2's "Rejected: Clamping `dbArrivalFactor` at 1" bullet is historically accurate ONLY for round 2's choice — round 4 (§63) adopted the clamp. A one-line cross-ref keeps the trail chronological without rewriting history.
- **§61 Known-limitation paragraph** (added by round-7 Item 3 in `08e46e2`) is also superseded for the fanout-write case. The genuine documented residual is now ONLY heterogeneous-DYNAMIC-filter routes (one through a cache, one direct), where per-side attribution can still drift even though aggregate stays bounded by `totalInboundRps`. Updated wording in §61 reflects this narrower scope.
- **Source:** [src/engine/SimulationEngine.ts](src/engine/SimulationEngine.ts) `routeStaticAmplificationToNode` (new), `computeDbArrivalFactor` (uses amp in denominator), `computeDbReadWriteBreakdown` (uses amp in per-route dbShare), `processDatabase` scan path (same amp scaling). New regression test: [engineReadWriteSplit.test.ts](src/engine/__tests__/engineReadWriteSplit.test.ts) `fanout-amplified writes saturate the DB — aggregate errorRate reflects real load`. Full suite 421/421 + Playwright 2/2. Codex desktop output: [docs/codex-reviews/2026-04-27-phase4-final-convergence/OUTPUT.md](docs/codex-reviews/2026-04-27-phase4-final-convergence/OUTPUT.md).

### 67. Crashed backpressure targets keep signaling from their frozen errorRate

- **When:** 2026-06-10 (full-Playwright verification pass before Phase 8a; first full e2e run since Phase 4 began).
- **Context:** `e2e/simfid-phase3-backpressure.spec.ts` failed ~30% of runs on the Phase 4 branch (and, with different timing, on `main`). The scenario drives a DB at 40 RPS against 4 RPS capacity → CPU pegs at 100% from tick 0 → `updateComponentHealth` rolls a 30%-per-tick crash chance at >98% utilization. The end-of-tick acceptanceRate update had `if (state.crashed) return`, so a crash landing on tick 0 — before the FIRST evaluation — froze `acceptanceRate` at its init value 1.0 forever: a dead, 100%-saturated database advertising full acceptance. Upstream never scaled down, the "signaling backpressure" callout never fired, the spec failed. Phase 4's read/write split narrowed the window to one tick (errorRate ≈0.9 already at tick 0) but the race predates Phase 4.
- **Decision:** Remove the crashed-skip from the acceptanceRate update. Crashed components keep last-known metrics (§12c), so the signal now derives from that frozen aggregate `errorRate` — for a saturated-then-crashed DB, acceptance ≈ 0.05–0.1 and upstream sheds ~90–95% with the callout firing on the next emit. Consistent with §52: the control signal still reads only the aggregate errorRate (frozen or live).
- **Why:** "Dead component accepts everything" is indefensible modeling — it is the one state where acceptance is most obviously NOT 1.0. Deriving from frozen metrics keeps continuity with how breakers already treat crashed targets (they read the frozen `errorRate` via Phase C, which holds the last value under the rps>0 guard). Also makes the e2e spec deterministic: 6/6 consecutive green after the fix, seed-pinned unit regression red-before/green-after.
- **Rejected:** Forcing `acceptanceRate = 0` on crash (strongest semantics, but silently drops all forwarded traffic, which would mute the error signals breakers and retry storms feed on — a dead DB would make the upstream look calm). Fixing only the test (seeding the e2e engine) — leaves the modeling flaw in the product. Reordering crash check after the BP update (fixes only the tick-0 case; a crash between later evaluations would still freeze a stale-but-recent value, which the frozen-metrics derivation handles uniformly).
- **Source:** [src/engine/SimulationEngine.ts](src/engine/SimulationEngine.ts) end-of-tick acceptanceRate loop (crashed-skip removed, comment explains). Regression test: [SimulationEngine.test.ts](src/engine/__tests__/SimulationEngine.test.ts) `a target that crashes on tick 0 still signals backpressure from its frozen errorRate` (seed 4 pinned; the crash assertion is pinned to TICK 0's log batch so an RNG-order change can't make the test evergreen). Same verification pass also fixed two pre-existing reds outside the engine: the WikiRoute duplicate tab bar resurrected by merge `4f761bc` (7 docs/wiki specs), and all 53 `tsc --noEmit` errors (`SimComponentData` interface → type alias for ReactFlow v12's `Node<T extends Record<string, unknown>>` constraint — type literals satisfy it via implicit index signature without losing excess-property checks; dead `src/main.ts` deleted; `@types/node` added). Full suite after: vitest 422/422, Playwright 99/99, `tsc` 0 errors.
- **Known limitation (adversarial review, 2026-06-10 — deferred):** the fix covers crashes whose frozen `errorRate` is non-zero (the saturated-DB case). A component can also crash in the **errorRate = 0 window** — e.g. utilization between 98–100% where `saturationErr` is still 0, or a hot-shard MEMORY crash — and then keeps advertising `acceptanceRate = 1.0` from its frozen zero errorRate. Two adjacent pre-existing behaviors share the root cause "crashed components freeze whatever errorRate they died with": a breaker can CLOSE through HALF_OPEN onto a dead target whose frozen errorRate is 0, and the autoscaler scales up crashed downstreams from their frozen `cpuPercent`. The clean fix is one decision — "crashed = errorRate 1.0 at crash time" (still aggregate-only per §52: breakers would OPEN onto dead targets, retry/backpressure would react) — but it changes breaker/retry semantics for every crash in every existing scenario and deserves its own reviewed pass with test sweep, not a rider on this fix. Deferred to a dedicated entry.

### 68. SIMFID Phase 8a — BOTE estimator in the empty inspector dock + calibration.json scaffold with null anchors

- **When:** 2026-06-10 (Phase 8a, branch `feat/simfid-phase8a-bote-calibration` stacked on the Phase 4 branch).
- **Context:** Plan §8a ships two things ahead of Phase 5: a back-of-the-envelope capacity panel (DAU → QPS → storage → connections) and the calibration.json schema the Phase 5 daemon harness will emit, loaded with empty defaults so nothing changes until real measurements exist.
- **Decision:** (1) **Placement** — BotePanel renders in the right inspector dock when an explicit `botePanelOpen` store flag is set AND no node/wire is selected (per spec "inspector panel when no node is selected"); opened via the Traffic tab's "Pre-populate from capacity estimator →" button; selecting a node clears the flag. *(Adversarial review P1: the original "configPanelOpen with no selection" condition surfaced the panel uninvited on every stale-selection path — Delete Component, Backspace delete, undo — and in scenario mode let Apply clobber the scenario's traffic profile. The explicit flag closes all of these: scenario mode can't reach the panel because the opener lives in the freeform-only Traffic tab.)* (2) **Inputs are store-resident** (`boteInputs`), not panel-local — ConfigPanel unmounts the panel on node selection, and losing half-typed DAU on a stray canvas click reads as a bug; §44's panel-local precedent covers view filters, not user-entered data. (3) **Apply semantics** — `toTwoPhaseProfile` replaces `phases` (steady baseline at avg QPS for ~2/3 of the run, then a spike to peak) but preserves the existing profile's `durationSeconds`/`requestMix`/`userDistribution`/`jitterPercent`; Apply is disabled mid-run (the engine snapshots the profile at start, so a mid-run apply would only LOOK applied); TrafficEditor gains an effect re-seeding its local draft whenever `trafficProfile` changes so the applied profile is immediately visible/editable. (4) **Calibration mapping** — anchors feed ONLY config-default sites that exist today: postgres read/write throughput → DB defaults (50k/20k), fastify `serviceTimeMs.p50` → server `processingTimeMs` default (50), fastify `serviceVariance` → Kingman C_s² default (1.0). The redis profile ships schema-only — the cache model has no calibrated-latency knob, and inventing one for Phase 8a would be scope creep. (5) **Loading** — engine takes an optional trailing `calibration` constructor param (additive; absent = exact pre-8a behavior); `useSimulation` primes a fire-and-forget fetch on mount and passes whatever loaded by Run time. With all-null shipped anchors the `??` chains are bit-identical to the hard-coded defaults (pinned by test). Boundary hardening from adversarial review: anchors must be strictly positive (a calibrated `serviceTimeMs.p50: 0` would yield `1000/0 = Infinity` RPS in QueueingModel; negative throughput → negative utilization), and a file whose body claims a different `primitive` than its filename is dropped rather than stored under the wrong slot. (6) **Branching** — stacked on the Phase 4 branch rather than `main` because the calibration serviceVariance default lands inside Phase 4's Kingman site; the plan's "parallel branch" note predates Phase 4's completion.
- **Why:** Highest credibility-per-line item in the research ("the credibility-per-LOC champion"); 8a blocks Phase 5 (the harness emits into this schema, not the other way around). Empty-defaults-with-fallbacks is the established opt-in pattern that has protected the suite through three phases.
- **Rejected:** Sidebar tab for the panel (Traffic-tab precedent fits global state, but the spec names the inspector and the empty dock was dead space). Awaiting the calibration fetch before first Run (blocks the hot path for a file that ships empty; revisit when Phase 5 writes real anchors — the pill UI lands there). Mapping redis anchors onto `WorkingSetCache` constants now (no config knob exists; would invent an engine surface Phase 5 may shape differently). Kafka profile (deferred per plan §8a until a user names a Kafka-specific failure mode).
- **Source:** [src/util/bote.ts](src/util/bote.ts) (math + two-phase projection), [src/components/panels/BotePanel.tsx](src/components/panels/BotePanel.tsx), [ConfigPanel.tsx](src/components/panels/ConfigPanel.tsx) (no-selection branch), [TrafficEditor.tsx](src/components/panels/TrafficEditor.tsx) (open button + draft re-seed effect), [src/store/index.ts](src/store/index.ts) `boteInputs`/`setBoteInputs`, [src/engine/calibration.ts](src/engine/calibration.ts) (types, loader, module cache), [SimulationEngine.ts](src/engine/SimulationEngine.ts) (constructor param + three `??` sites), [useSimulation.ts](src/engine/useSimulation.ts) (prime + pass), `public/calibration/laptop-m-series-16gb/{postgres-16,redis-7,fastify-5}.json`. Tests: [bote.test.ts](src/util/__tests__/bote.test.ts) (12), [calibration.loader.test.ts](src/engine/__tests__/calibration.loader.test.ts) (13), [e2e/simfid-phase8a-bote.spec.ts](e2e/simfid-phase8a-bote.spec.ts) (3, screenshots in test-results/simfid-phase8a-bote/). Vitest 447/447.

### 69. Docs completion — topic back-fill in per-category content modules, KB §45–48, credibility footers

- **When:** 2026-06-10 (user directive: "finish the docs completely using the knowledgebase").
- **Context:** The Phase A-content back-fill that topics.ts's own header promised never happened — 99 of ~104 hand-declared wiki topics were `empty()` placeholders, so every InfoIcon in the app showed "Documentation coming soon." The KB's Part VIII stopped at §44, predating Phase 4's engine features and Phase 8a entirely. TODOS' [POST-WIKI] credibility pass (primary-source footers) was also outstanding.
- **Decision:** (1) **Content modules, not a fatter registry** — topic bodies live in `src/wiki/content/topics/{componentsAndConcepts,configsInfra,configsDataTraffic,newFeatures}.ts`, merged into `TOPICS` after the empty declarations and before the generated merges (generated still wins for `howto.*`, whose bodies come from markdown). Keys/titles stay canonical in topics.ts; content modules override. This let five writer agents work in parallel with zero same-file conflicts and keeps diffs reviewable. (2) **Honesty-first config docs** — bodies explicitly distinguish engine-live fields (with defaults and the consuming code path) from declared-but-ignored fields (`algorithm`, health-check fields, `writeStrategy`, `retryCount`, `backpressure.thresholdQueue`, `traffic.jitterPercent`, `traffic.userDistribution`, …) which are documented as design-intent/reserved rather than pretending the engine reads them. (3) **KB §45–48** extend Part VIII for the shipped engine: Kingman G/G/1 (§45), DB read/write split + attribution (§46), capacity estimation (§47), calibration (§48) — every claim verified against code by the authoring agent, honest-fidelity caveats stated. (4) **Credibility footers** on §1–§39: single italic `*Sources: …*` line per section, canonical books/papers/RFCs/official docs by NAME ONLY, no URLs (URLs rot and invite fabrication); case-study sections honestly footered as "practitioner case study (adapted)". (5) New Learn page 19 (capacity estimator), how-to 06 (capacityPlanning, with template numbers verified to produce the saturation the prose narrates), `config.bote.*` + `config.serviceVariance` topics, InfoIcons wired into BotePanel.
- **Why:** The docs/wiki product is the retention hook (PRODUCT_VISION: complete component wiki, "do not miss anything") and InfoIcon tooltips were the most visible broken promise in the app. Module split is the only shape that parallelizes content authorship safely. The no-URL citation rule is the difference between a credibility pass and a liability pass.
- **Rejected:** Back-filling topics.ts in place (single 150KB file, serial authorship, unreviewable diff). Auto-generating component/config bodies from the KB (the KB teaches concepts; tooltips must describe ENGINE behavior — different content with different truth conditions). URL citations (rot + fabrication risk). Documenting `severity.error` as a live level (nothing emits it — documented as reserved instead).
- **Source:** [src/wiki/topics.ts](src/wiki/topics.ts) (merge block), [src/wiki/content/topics/](src/wiki/content/topics/) (4 modules; 102 agent-written bodies + `concept.p50p95p99` added at assembly), [system-design-knowledgebase.md](system-design-knowledgebase.md) §45–48 + 34 source footers, [src/wiki/content/learn/19-capacity-estimator.md](src/wiki/content/learn/19-capacity-estimator.md), [src/wiki/content/howto/06-capacityPlanning.md](src/wiki/content/howto/06-capacityPlanning.md), [public/templates/howto/capacityPlanning.json](public/templates/howto/capacityPlanning.json), [BotePanel.tsx](src/components/panels/BotePanel.tsx) InfoIcons. Registry state after: 170 topics, 0 empty. Generator: 43 reference + 19 userGuide + 6 howto.

### 70. Design review — templates ship run-ready via a `starter` block; the 0-to-1 fix series

- **When:** 2026-06-10 (user directive: full adversarial UX audit via /design-review; "is it really 0-to-1 or complexity masquerading as features?").
- **Context:** A live walkthrough of every entry path (browse-driven, screenshots in `test-results/design-audit/`) found the surfaces clean but the FLOW broken at each entry: beginner templates landed on a disabled Run plus a 3-item preflight homework list; the guided scenario dead-ended on a blank canvas with zero guidance; the debrief opened as a 180px strip scrolled past its own scores and told a crashed system it "held up well"; dark mode had a hard-white minimap; the log's grouping could mathematically never fire (2s window vs 2s throttle); crashed nodes wore a close-button "✕". Audit report: `~/.gstack/projects/T-98-systemsim/designs/design-audit-20260610/` (Design Score B−, goodwill trace 70→35 across the 0-to-1 flow).
- **Decision:** (1) **Template JSON gains an optional `starter` block** (`trafficProfile` + `schemaMemory` + `apiContracts`) applied by TemplatePicker after `replaceGraph` — node references use replaceGraph's deterministic `${type}-${index}` ids; all 6 templates authored with capacity-sane numbers (baseline ~50–80% of the bottleneck, spike past 100%). Templates are now "click → Run"; preflight remains the gate for user-authored designs. This is a format contract: future templates and Phase 7's exporter must follow it. (2) Twelve `style(design): F-NN` commits fix the rest — empty-canvas mission card (F-02), debrief auto-expand + scroll-reset + severity-aware fallback question (F-03/F-04), themed translucent minimap (F-05/F-08), CRASHED pill (F-06), log group window 12s + severity glyphs (F-07), truthful AI-failure copy (F-09), preflight deep-link auto-expands the traffic editor (F-10), labeled grids (F-11), Design Flow header + honest title (F-12), docs single-title + echo-lede suppression (F-13), BOTE pinned summary footer (F-14), landing eyebrow/hit-targets (F-15/F-17), live "Load in canvas" how-to button replacing the forgotten disabled stub (F-19).
- **Why:** The product's promise is "watch where it breaks" in under a minute; every entry path violated it before first dopamine. Run-ready templates fix the funnel without weakening the schema-driven rigor — the rigor stays as the upgrade path, not the toll booth.
- **Rejected:** Visual reskin (the aesthetic was never the problem — Apple-clean, AI-slop grade A−). Auto-generating starter state at load time from heuristics (silently invents schema the user didn't author; a `starter` block is explicit, versioned template content). Removing preflight for templates (preflight is correct for user-authored designs; templates simply arrive complete).
- **Deferred (logged in TODOS):** F-18 debrief "no indexes defined" false positive (deterministic check reads node config, not schemaMemory — engine backlog); speed control visible pre-run; "Particle" toggle label; green Debrief button tonality after catastrophic runs.
- **Source:** [TemplatePicker.tsx](src/components/ui/TemplatePicker.tsx) starter application, `public/templates/*.json` (all 6), [CanvasEmptyState.tsx](src/components/canvas/CanvasEmptyState.tsx) (new), [BottomPanel.tsx](src/components/panels/BottomPanel.tsx), [useSimulation.ts](src/engine/useSimulation.ts), [debrief.ts](src/ai/debrief.ts) (+ regression test `never says "held up well" after a crash`), [SimComponentNode.tsx](src/components/nodes/SimComponentNode.tsx), [groupLogs.ts](src/components/panels/liveLog/groupLogs.ts) + [LogGroupedRow.tsx](src/components/panels/liveLog/LogGroupedRow.tsx), [index.css](src/index.css), [Canvas.tsx](src/components/canvas/Canvas.tsx), [UnifiedInput.tsx](src/components/ui/UnifiedInput.tsx), [LandingPage.tsx](src/components/ui/LandingPage.tsx), [TrafficEditor.tsx](src/components/panels/TrafficEditor.tsx), [DesignFlow.tsx](src/components/panels/DesignFlow.tsx), [TopicBody.tsx](src/wiki/components/TopicBody.tsx), [BotePanel.tsx](src/components/panels/BotePanel.tsx).

### 71. Chaos injection — kill/revive components mid-run (the spark demo)

- **When:** 2026-06-11 (user directive: build the single "WOW" use case — live, under a minute, genuinely impressive).
- **Context:** The validated moat is runtime fidelity, and the most visceral possible proof is letting the user hurt a running system with their finger. PRODUCT_VISION's wishlist already named it ("test 'oh shit' moments"). All the failure plumbing existed — organic crashes (§12c frozen metrics), LB skip-crashed split, breaker state machines, frozen-aggregate backpressure (§67) — what was missing was a way to TRIGGER a crash deliberately and to undo it.
- **Decision:** (1) **Engine API**: `injectCrash(id)` / `revive(id)` — a manual kill is byte-identical to an organic crash (same `crashed` flag, same frozen-metrics semantics), so every §52-honoring control signal reacts without new code paths. Revive resets metrics and acceptanceRate; wires with OPEN breakers re-trust on their own cooldown→HALF_OPEN-probe→CLOSED clock, which IS the recovery half of the demo. (2) **Log queue**: UI handlers fire between ticks, so chaos events queue in `pendingChaosLogs` and drain unthrottled with the next tick's batch ("CHAOS — X killed manually… Watch the survivors."). (3) **UI bridge**: the engine instance is private to the useSimulation hook instance that started the run, but kill/revive affordances live on canvas nodes — a module-level `chaosHandle` (nulled on stop/teardown) bridges them; exposed as `window.__SYSTEMSIM_CHAOS__` for Playwright (store-exposure precedent §5). (4) **Affordance**: during a run, hover shows a bordered red **Kill** pill where the info icon sits pre-run; a crashed node's CRASHED pill becomes **Revive** (green) on hover. (5) **Demo asset**: `chaos_drill` template (run-ready per §70's starter contract) — two replicas at 75% utilization behind an LB with retries + breakers; killing one sends the survivor to 150%, retry amplification pushes it past the breaker threshold, and in most seeds the survivor organically crashes — a true metastable cascade. The e2e had to scope assertions per-node because of it.
- **Why:** Every alternative spark (text→diagram needs a live LLM round-trip; cost-slider lacks drama; A/B comparison needs concurrent engines) is either riskier on stage or less visceral. This one is deterministic, offline, interactive, and demos ONLY the thing competitors can't copy: the boxes actually run.
- **Rejected:** Modeling the kill as a distinct "chaos" state with special signal handling (organic-crash parity is the honesty guarantee — if the cascade differs from a real crash, the demo is a lie). Right-click context menu (hidden affordance; hover pill is discoverable). Killing via ConfigPanel (too slow for stage). Putting the engine handle in the zustand store (engine refs in the store invite render-loop coupling; a nulled module handle is simpler and §49's scoped-engine refactor will subsume it).
- **Known limitation:** revive resets the component's metrics wholesale; a revived DATABASE forgets accumulated connections (treats revival as a cold restart — defensible, but stated). Kill while PAUSED queues the log until resume.
- **Source:** [SimulationEngine.ts](src/engine/SimulationEngine.ts) `injectCrash`/`revive`/`pendingChaosLogs` + tick drain, [useSimulation.ts](src/engine/useSimulation.ts) `chaosHandle` lifecycle, [SimComponentNode.tsx](src/components/nodes/SimComponentNode.tsx) Kill/Revive pills, [public/templates/chaos_drill.json](public/templates/chaos_drill.json) + index entry. Tests: 3 chaos unit tests in [SimulationEngine.test.ts](src/engine/__tests__/SimulationEngine.test.ts), [e2e/chaos-drill.spec.ts](e2e/chaos-drill.spec.ts) (3 specs, demo-arc screenshots). Vitest 456/456.

### 72. Drills — learn-by-fixing challenges, engine-verified content, FlowDiagram docs tag

- **When:** 2026-06-11 (user directive: diagrams for Reference topics + "re-create a scenario in real time on canvas and challenge the user to diagnose from the log and fix the structure/config" + a Learn entry on the landing page — the product wedge for learners).
- **Context:** The docs taught theory and the simulator ran designs, but nothing connected them into the learning loop that makes skills stick: see a failure → name its cause → change the design → prove the fix. PRODUCT_VISION's coaching thesis ("users keep coming back because they learn AND apply") had no mechanism.
- **Decision:** (1) **Challenge contract** (`public/challenges/<id>.json`): broken `CanonicalGraph` + §70-style starter + a 3-step pedagogy — observe (auto-run on load so the failure is LIVE within seconds), diagnose (multiple-choice with one correct option; every option's `explain` teaches, checkable without AI), fix (user edits config/structure, re-runs; machine-checkable `criteria` evaluate every completed run). (2) **Criteria primitives** kept deliberately small — metric max/avg over a component selector with an optional warm-up window, plus `noCrash` — pure functions over the run artifact (`challenges/evaluate.ts`), no AI, no network, no flake. (3) **`knownFix` is part of the contract**: the content test harness (`challengeContent.test.ts`) runs the REAL engine over every shipped challenge at two seeds and requires broken-FAILS + knownFix-PASSES — an unsolvable or non-broken drill cannot ship. All 10 launch drills (§9–§24 coverage: SPOF, rate limit, stampede, queue backlog + overflow, missing cache, pool exhaustion, hot shard, retry storm, fanout flood) were authored by parallel agents iterating against this gate; every number traces to engine formulas. (4) **Auto-run bridge**: the loader raises `autoRunRequested` in the store; Toolbar's useSimulation instance (the only engine owner) watches and fires — same bridge family as §71's chaosHandle. (5) **HUD** (`ChallengeBanner`) owns step state and scoring; `replaceGraph` exits any active drill. (6) **Landing "Learn — fix a production fire"** card grid between templates and the guided scenario. (7) **`<FlowDiagram chain="type:Label -> …"/>`** markdown tag (CanvasEmbed splice mechanism, char-allowlisted attribute) renders canvas-consistent component boxes — 26 diagrams inserted across 23 KB sections. (8) **Toolbar "Run again"** in the completed state — the edit→re-run fix loop had a dead Reset click in it. (9) **Engine**: crash logs register as callouts so the per-(component,severity) throttle can never swallow them — found by the harness when a same-tick pool-exhaustion critical ate a CRASH line, which would have blinded `noCrash` criteria and users alike.
- **Why:** This is the wedge: competitors can render text about backpressure; only a runtime can drop you into a failing system and referee your fix. The engine-verified content gate is what makes scaling to "almost every section" trustworthy — pedagogy backed by the same math the product sells.
- **Rejected:** LLM-judged diagnosis/fix (non-deterministic, needs network/key, unverifiable content at scale — MCQ + metric criteria are checkable forever). Free-form diagnosis text input (same problem). Auto-applying the fix for the user ("show answer" can come later from `knownFix`, but the drill's value is the user doing it). Mermaid for diagrams (foreign visual language; FlowDiagram reuses canvas icons/colors so docs and product look like one thing). Embedding drills inside docs pages first (needs the §49 scoped-engine refactor; canvas-first ships now, docs links later).
- **Known limitation:** criteria read `metricsTimeSeries` indexed by tick ≈ sim-second; if tick semantics ever change, `windowStartS` needs revisiting. Diagnosis options are static per challenge (no randomization yet).
- **Source:** [src/challenges/](src/challenges/) (types, evaluate, harness, loadChallenge + tests), [ChallengeBanner.tsx](src/components/canvas/ChallengeBanner.tsx), [LearnSection.tsx](src/components/ui/LearnSection.tsx), [Toolbar.tsx](src/components/ui/Toolbar.tsx) (auto-run effect + Run again), [store/index.ts](src/store/index.ts) (drill slice), [FlowDiagram.tsx](src/wiki/components/FlowDiagram.tsx) + [MarkdownBody.tsx](src/wiki/components/MarkdownBody.tsx) (tag splice), [SimulationEngine.ts](src/engine/SimulationEngine.ts) (crash-callout), `public/challenges/*.json` (10 + index), [system-design-knowledgebase.md](system-design-knowledgebase.md) (26 diagrams). Tests: content gate 31, evaluator/parser 5, [e2e/learn-drills.spec.ts](e2e/learn-drills.spec.ts) (4 specs, full-arc screenshots). Vitest 499/499.
