...
- [x] **Codex round 4 (2026-04-24) — Decisions §63.** Fourth review pass found two more: [P1] `dbArrivalFactor` uncapped — when `requestMix` was partially routed, default-bucket traffic got absorbed into routed attribution (a DB with 10% routed writes + 90% default would be simulated as 100% writes). [P2] `useSimulation.ts` was deriving `schemaShardKey` / `schemaShardKeyCardinality` from the first schemaMemory entity and passing them as constructor globals — `resolveShardKeyForDb`'s global fallback then let one DB inherit another DB's partition key. Fix: (1) `Math.min(1, totalInboundRps / entryShareToDb)` — routes can only scale DOWN; (2) `useSimulation` always passes `undefined` for the legacy globals when `routingContext.schemaMemory` is set. Two new regression tests. Full suite 414/414 + Playwright 2/2 green.
- [x] **Codex round 5 (2026-04-24) — Decisions §64.** Fifth review pass caught two edit-flow correctness gaps: [P1 seed-chain] `seedInboundTraffic` only validated `componentChain[0]` exists, not that the whole chain is still a connected path — a stale chain post-graph-edit would inject traffic past newly-added upstream gateways/LBs (their rate limits skipped). [P2 stressed-Ca²] `getCurrentArrivalVariance()` looked up the current-time phase's shape, but stressed mode runs at peak RPS across all phases — early ticks could use steady-phase Cₐ²=1 while running at peak spike RPS, Kingman underestimating the worst-case wait. Fix: (1) seed walks consecutive pairs in `componentChain` against live `adjacency`, fires `routing-stale:<endpointId>` + redistributes share if any pair is broken; (2) stressed-mode Cₐ² comes from the peak-RPS phase's shape (highest-variance tie-break). Two new regression tests. Full suite 416/416 + Playwright 2/2 green.
- [x] **Codex round 6 (2026-04-24) — Decisions §65.** Sixth pass covered three remaining gaps: [P1 ingress-bypass] round-5's chain-pair validation didn't catch single-node chains `['svc']` where the head had predecessors added later (the chain has no pairs to check). [P2 shard regression] round-4's `undefined`-global fix also broke single-DB hot-shard for unassigned-schema saves (pre-DB-assignment step). [P2 deferred CO] `WireTickOutcome.dispatchedAtTickMs` was stamped at emit time, so deferred paths reported wrong age on the next tick. Fixes: (1) head-has-predecessors-and-not-entry → stale; (2) useSimulation restores legacy globals ONLY when no entity has `assignedDbId`; (3) new `componentEarliestInboundMs` map propagates earliest dispatch through `pendingInbound`; emit stamps `Math.min(this.time*1000, earliest)`. Three new regression tests. Full suite 419/419 + Playwright 2/2 green.
- [x] **Codex round 7 (2026-04-24) — three findings split across three commits per the round-7 final handoff [`docs/plans/2026-04-24-simfid-phase4-final-handoff.md`](2026-04-24-simfid-phase4-final-handoff.md):**
  - `c667e73` **Item 1 [P1 LB-widget]** — `FanoutTailSection` was rendered for `load_balancer | fanout | api_gateway`. The Dean-Barroso compounding math is correct only for true scatter-gather (every backend hit per request); LBs route ONE request to ONE backend, so the widget claimed ~63% slow on a 100-backend LB at p=1% when the actual rate is ~1%. Gate narrowed to `data.type === 'fanout'` only. Decisions §58 round-7 paragraph appended.
